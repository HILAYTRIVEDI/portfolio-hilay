...
Use for portfolios, landing pages, and any site where the user wants instant preview with zero build step.
...
├── assets/images/
└── public/videos/      ← HyperFrames-rendered MP4/WebM
```
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .container { width: min(var(--content-width), 100% - 2rem); margin-inline: auto; }
...
<body>
  ...
  <nav aria-label="Main navigation"><div class="container"><!-- nav --></div></nav>
...
```css
.hero { position: relative; height: 100svh; display: flex; align-items: center; overflow: hidden; }
.hero-bg-video { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; z-index: 0; opacity: 0.4; }
...
.card-thumb, .card-video { width: 100%; aspect-ratio: 16/9; object-fit: cover; transition: opacity 0.3s ease; }
.card-video { position: absolute; inset: 0; height: 100%; opacity: 0; }
...
gsap.matchMedia().add("(prefers-reduced-motion: no-preference)", () => {
  gsap.from('.skill-tag', {
    opacity: 0, scale: 0.85, y: 10, duration: 0.4, stagger: 0.04, ease: 'back.out(1.4)',
    scrollTrigger: { trigger: '.skills-grid', start: 'top 80%' }
  });
});
```

### Magnetic hover (social links)
```js
